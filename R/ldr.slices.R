...
{	
	...
	{		
		...
		{ 
			bins.y[[i]] <- sorty[(ends[i-1]+1):ends[i]];

			indicators[(ends[i-1]+1):ends[i]] <- i;
		}
		bins.y[[nslices]] <- sorty[(ends[nslices-1]+1):(ends[nslices])];

		indicators[(ends[nslices-1]+1):(ends[nslices])] <- nslices;
