pfc <-
function(X, fy=fy, numdir=2, structure=c("iso", "aniso", "unstr", 
	"iso2", "aniso2", "un.iso", "un.aniso", "unstr2"), eps_aniso=1e-2, 
	...
	if (structure=="iso2")
	{
		iso2 <- function(i)
		{
			objfun <- function(W)
			{
				...
				value <- -(n/2)*(p*log(2*pi)+p+(p-dc)*log(Trace(t(V)%*%S$Sigmahat%*% V)/(p-dc))+
						dc*log(Trace(t(U)%*%S$Sigmahat_res %*% U)/dc));

				terme1 <- 2*(p-dc)*t(U) %*% S$Sigmahat %*% V/Trace(S$Sigmahat - S$Sigmahat%*%U%*%t(U))

				terme2 <- 2*dc* t(U) %*% S$Sigmahat_res %*% V/Trace(t(U) %*% S$Sigmahat_res %*% U); 

				gradient <- terme1 - terme2;
			...
			W <- list(Qt = svd(Sigmahat_fit)$u, dim=c(numdir, p), Sigmas=list(Sigmahat=Sigmahat, 
					Sigmahat_fit=Sigmahat_fit, Sigmahat_res=Sigmahat_res, p=p, n=n));
			...
			Omegahat <- (Trace(t(Gammahat)%*%Sigmahat_res%*%Gammahat)/i)*diag(i);

			Omegahat0 <-(Trace(t(Gammahat0)%*%Sigmahat%*%Gammahat0)/(p-i))*diag(p-i);
			...
			temp1 <- -(n/2)*i*log(Trace(t(Gammahat)%*%Sigmahat_res%*%Gammahat)/i); 

			temp2 <- -(n/2)*(p-i)*log(Trace(t(Gammahat0)%*%Sigmahat%*%Gammahat0)/(p-i)); 

			loglik <- temp0 + temp1 + temp2;  

			numpar <- p + (p-i)*i + i*dim(fy)[2] + 1 + 1; 
			...
			return(list(Betahat=Betahat, Gammahat=Gammahat, Gammahat0=Gammahat0, Deltahat=Deltahat, Omegahat=Omegahat, 
					Omegahat0=Omegahat0, evalues=evalues, loglik=loglik, aic=aic, bic=bic, numpar=numpar));
		}
		if (identical(short, TRUE))
		{
			out <- iso2(numdir);

			ans <- list(Muhat=Muhat, Betahat=out$Betahat, Gammahat=out$Gammahat, Gammahat0=out$Gammahat0, Deltahat=out$Deltahat, 
					Omegahat=out$Omegahat, Omegahat0=out$Omegahat0, evalues=out$evalues, loglik=out$loglik, aic=out$aic, bic=out$bic, 
					numpar=out$numpar, numdir=numdir, model="pfc", structure="iso2", fy=fy, Xc=Xc,  call=match.call(), short=short);
		...
		ans <- list(evalues=fit$evalues, loglik =loglik, aic=aic, bic=bic, numdir=numdir, numpar=numpar, Muhat=Muhat, 
				Betahat=Betahat, Gammahat=Gammahat, Gammahat0=Gammahat0, Omegahat=Omegahat, Omegahat0=Omegahat0, 
				Deltahat=Deltahat, model="pfc", structure="iso2", fy=fy, Xc=Xc,  call=match.call(), short=short);

		class(ans)<- "ldr";

		return(ans)
	}
	else if (structure=="aniso2")
	{
		dia2 <- function(i)
		{
			objfun <- function(W)
			{
				...
				#value <- -(n/2)*(p*log(2*pi)+p+log(prod(diag(t(V)%*%S$Sigmahat%*% V)))+log(prod(diag(t(U)%*%S$Sigmahat_res %*% U))));
			...
			sigmas <- list(Sigmahat=Sigmahat, Sigmahat_fit=Sigmahat_fit, Sigmahat_res=Sigmahat_res, 
					p=p, n=n);
			...
			Omegahat <- diag(t(Gammahat)%*%Sigmahat_res%*%Gammahat)*diag(i);

			Omegahat0 <-diag(t(Gammahat0)%*%Sigmahat%*%Gammahat0)*diag(p-i);
			...
			temp1 <- -(n/2)*log(prod(diag(t(Gammahat)%*%Sigmahat_res%*%Gammahat))); 

			temp2 <- -(n/2)*log(prod(diag(t(Gammahat0)%*%Sigmahat%*%Gammahat0))); 

			loglik <- temp0 + temp1 + temp2;  

			numpar <- p + (p-i)*i + i*dim(fy)[2] + p; 
		...
		if (identical(short, TRUE))
		{
			...
			ans <- list(Muhat=Muhat, Betahat=out$Betahat, Gammahat=out$Gammahat, Gammahat0=out$Gammahat0, Deltahat=out$Deltahat, 
					Omegahat=out$Omegahat, Omegahat0=out$Omegahat0, evalues=out$evalues, loglik=out$loglik, aic=out$aic, bic=out$bic, 
					numpar=out$numpar, numdir=numdir, model="pfc", structure="aniso2", fy=fy, Xc=Xc,  call=match.call(), short=short);
		...
		ans <- list(evalues=fit$evalues, loglik =loglik, aic=aic, bic=bic, numdir=numdir, numpar=numpar, Muhat=Muhat, 
				Betahat=Betahat, Gammahat=Gammahat,	Gammahat0=Gammahat0, Omegahat=Omegahat, 
				Omegahat0=Omegahat0, Deltahat=Deltahat, model="pfc", structure="aniso2", fy=fy,  Xc=Xc, call=match.call(), short=short);

		class(ans)<- "ldr";

		return(ans)
	}
	else if (structure=="un.iso") 
	{
		unstr.iso <- function(i)
		{
			objfun <- function(W)
			{
				Qt <- W$Qt; dc <- W$dim[1]; 
				...
				U <- matrix(Qt[,1:numdir], ncol=dc);	

				V <- matrix(Qt[,(numdir+1):p], ncol=(p-dc));

				value <- -(n/2)*(p*log(2*pi)+p+(p-dc)*log(Trace(t(V)%*%S$Sigmahat%*%V)/(p-dc))+ log(det(t(U)%*%S$Sigmahat_res%*%U)));

				terme1 <- (p-dc)*t(U) %*% S$Sigmahat %*% V/Trace(S$Sigmahat - S$Sigmahat%*%U%*%t(U));

				terme2 <- solve(t(U)%*% S$Sigmahat_res%*%U)%*%t(U)%*%S$Sigmahat_res%*%V;
			...
			Omegahat0 <-diag(p-i)*Trace(t(Gammahat0)%*%Sigmahat%*%Gammahat0)/(p-i);
			...
			numpar <- p + (p-i)*i + i*dim(fy)[2] + i*(i+1)/2 + 1; 
		...
		if (identical(short, TRUE))
		{
			out <- unstr.iso(numdir);

			ans <- list(Muhat=Muhat, Betahat=out$Betahat, Gammahat=out$Gammahat, Gammahat0=out$Gammahat0, Omegahat=out$Omegahat,  
					Omegahat0=out$Omegahat0, Deltahat=out$Deltahat, evalues=out$evalues, loglik=out$loglik, aic=out$aic, bic=out$bic, numpar=out$numpar, 
					numdir=numdir, model="pfc", structure="un.iso", fy=fy, Xc=Xc,  call=match.call(), short=short);
		...
		for(m in 1:numdir)
		{
			...
			Betahat[[m]] <-fit$Betahat; Gammahat[[m]] <-fit$Gammahat; 
	
			Deltahat[[m]] <- fit$Deltahat; 

			Omegahat[[m]] <- fit$Omegahat; Omegahat0[[m]] <- fit$Omegahat0;

			loglik[m+1] <- fit$loglik; numpar[m+1] <- fit$numpar;

			aic[m+1] <- fit$aic; bic[m+1] <- fit$bic;	
		}
		ans <- list(evalues=fit$evalues, loglik =loglik, aic=aic, bic=bic, numdir=numdir, numpar=numpar, Muhat=Muhat, 
				Betahat=Betahat, Gammahat=Gammahat,	Gammahat0=Gammahat0, Omegahat=Omegahat, 
				Omegahat0=Omegahat0, Deltahat=Deltahat, model="pfc", structure="un.iso", fy=fy, Xc=Xc,  call=match.call(), short=short);

		class(ans)<- "ldr";

		return(ans)
	}
	else if (structure=="un.aniso") 
	{
		unstr.aniso <- function(i)
		{
			objfun <- function(W)
			{
				Qt <- W$Qt; numdir <- W$dim[1]; 

				p <- ncol(Qt); S <- W$Sigmas;
 
				U <- matrix(Qt[,1:numdir], ncol=numdir);	

				V <- matrix(Qt[,(numdir+1):p], ncol=(p-numdir));

				value <- -(n/2)*(p*log(2*pi)+p+log(prod(diag(t(V)%*%S$Sigmahat%*%V)))+ log(det(t(U)%*%S$Sigmahat_res%*%U)));
			...
			Omegahat0 <-diag(p-i)*diag(t(Gammahat0)%*%Sigmahat%*%Gammahat0);
			...
			numpar <- p + (p-i)*i + i*dim(fy)[2] + i*(i+1)/2 + p; 
		...
		if (identical(short, TRUE))
		{
			out <- unstr.aniso(numdir);

			ans <- list(Muhat=Muhat, Betahat=out$Betahat, Gammahat=out$Gammahat, Gammahat0=out$Gammahat0, Omegahat=out$Omegahat,  
					Omegahat0=out$Omegahat0, Deltahat=out$Deltahat, evalues=out$evalues, loglik=out$loglik, aic=out$aic, bic=out$bic, numpar=out$numpar, 
					numdir=numdir, model="pfc", structure="un.aniso", fy=fy, Xc=Xc,  call=match.call(), short=short);
		...
		ans <- list(evalues=fit$evalues, loglik =loglik, aic=aic, bic=bic, numdir=numdir, numpar=numpar, Muhat=Muhat, 
				Betahat=Betahat, Gammahat=Gammahat,	Gammahat0=Gammahat0, Omegahat=Omegahat, Omegahat0=Omegahat0, 
				Deltahat=Deltahat, model="pfc", structure="un.aniso", fy=fy, Xc=Xc,  call=match.call(), short=short);
