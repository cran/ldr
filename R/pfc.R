...
function(X, y, fy=NULL, numdir=NULL, structure=c("iso", "aniso", "unstr", "unstr2"), eps_aniso=1e-3, numdir.test=FALSE,...)
{
	...
	if (structure=="aniso")
	{
		aniso = function(X, y, fy, d, eps_aniso=1e-4, numdir.test)
		{
			vnames <- dimnames(X)[[2]];
		...
		if (numdir.test==FALSE) ans$R <- X%*%orthonorm(solve(fit$Deltahat)%*%fit$Gammahat) else 
			ans$R <- X%*%orthonorm(solve(fit$Deltahat[[eff.numdir]])%*%fit$Gammahat[[eff.numdir]]);
